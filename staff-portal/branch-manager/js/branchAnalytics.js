...
let unsubSales=()=>{}, unsubTop=()=>{}, unsubPay=()=>{}, unsubStock=()=>{}, unsubExp=()=>{};
...
const dateSpan = (s,e)=>{ const a=[]; const d=new Date(s);
  while(d<=e){ a.push(d.toLocaleDateString()); d.setDate(d.getDate()+1);} return a; };
...
async function bootstrapAnalytics () {
  ...
  /* 1️⃣  Wait for branchId via storage event (max 2 s) */
  if(!branchId){
...
async function attachListeners (branchId){
  ...
  const startDate = sTS.toDate(), endDate = eTS.toDate();
  ...
  unsubSales = onSnapshot(
    ...
    snap=>{
      const map={};
      snap.forEach(d=>{
        const dstr=d.data().createdAt.toDate().toLocaleDateString();
        map[dstr]=(map[dstr]||0)+d.data().grandTotal;
      });
      const labels = dateSpan(startDate,endDate);
      const data   = labels.map(l=>map[l]||0);
      redraw(chSales,'salesTrendChart','line',labels,
        [{label:'Daily Sales',data,borderColor:COLORS.salesLine,tension:.1,fill:false}],
        r=>chSales=r);
    });
  ...
  unsubTop = onSnapshot(
    ...
    snap=>{
      const qty=new Map();
      snap.forEach(d=>{
        (d.data().items||[]).forEach(({productId,qty:qt})=>{
          qty.set(productId,(qty.get(productId)||0)+qt);
        });
      });
  ...
  unsubPay = onSnapshot(
    ...
    snap=>{
      let cash=0,cred=0;
      snap.forEach(d=>{
        if(d.data().paymentType==='credit') cred += d.data().grandTotal||0;
        else                                cash += d.data().grandTotal||0;
      });
      redraw(chPay,'paymentTypeChart','doughnut',['Cash','Credit'],
        [{data:[cash,cred],backgroundColor:[COLORS.pieCash,COLORS.pieCred]}],
        r=>chPay=r,{plugins:{legend:{position:'bottom'}}});
    });
  ...
  unsubExp = onSnapshot(
    ...
    snap=>{
      ...
      snap.forEach(d=>{
        const dstr=d.data().createdAt.toDate().toLocaleDateString();
        map[dstr]=(map[dstr]||0)+d.data().amount;
      });
      const labels=dateSpan(startDate,endDate);
      const data  =labels.map(l=>map[l]||0);
      redraw(chExp,'expensesTrendChart','line',labels,
        [{label:'Daily Expenses',data,borderColor:COLORS.expLine,tension:.1,fill:false}],
        r=>chExp=r);
    });
}